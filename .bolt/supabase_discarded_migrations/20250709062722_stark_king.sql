/*
  ...
  2. New Table
    - Create `omniva_parcel_machines` table for storing parcel machine locations
      - Primary key: `zip`
      - Includes location data: country, county, city, district
      - Has updated_at timestamp with trigger
...
-- Create policy to allow public to read parcel machines (with IF NOT EXISTS)
CREATE POLICY IF NOT EXISTS "Enable public read access for omniva_parcel_machines" 
