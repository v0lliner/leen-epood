/*
  ...
  1. Function
    - `handle_product_sync_event()` - Automatically adds products to sync queue
    - Handles INSERT, UPDATE, DELETE operations
    - Only queues updates when relevant fields change
    - Stores metadata for delete operations

  2. Trigger
    - `products_stripe_sync_trigger` - Fires after product changes
    - Automatically queues products for Stripe sync

  3. Logic
    - INSERT: Queue as 'create' operation
    - UPDATE: Queue as 'update' only if relevant fields changed
    - DELETE: Queue as 'delete' with Stripe IDs in metadata
*/
...
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    -- Queue a delete operation for the product with Stripe IDs in metadata
