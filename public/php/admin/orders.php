...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        // If reference or order number is provided, get order details
        else if ($reference || $orderNumber) {
            $queryParam = $reference ? "reference=eq.$reference" : "order_number=eq.$orderNumber";
            $paramValue = $reference ?: $orderNumber;
            
            logMessage("Fetching order details for " . ($reference ? "reference" : "order number"), $paramValue);
            
            // Get order basic info
            $orderResult = supabaseRequest(
                "/rest/v1/orders?$queryParam&select=*,omniva_parcel_machine_id,omniva_parcel_machine_name,omniva_barcode,tracking_url,label_url,shipment_registered_at,reference",
                'GET'
            );
            
            if ($orderResult['status'] !== 200 || empty($orderResult['data'])) {
                logMessage("Order not found for " . ($reference ? "reference" : "order number"), $paramValue);
                http_response_code(404);
                echo json_encode([
                    'success' => false,
                    'error' => 'Order not found'
            ...
            logMessage("Fetching order items for order", $orderId);
            $itemsResult = supabaseRequest(
        // If we reach here, no specific order was requested, so get all orders
