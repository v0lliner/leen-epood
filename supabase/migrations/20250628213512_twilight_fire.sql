/*
  # Loo portfolio_items tabel tehtud tööde jaoks

  1. Uued tabelid
    - `portfolio_items`
      - `id` (uuid, primary key)
      - `title` (text, kohustuslik)
      - `technique` (text, valikuline)
      - `dimensions` (text, valikuline)
      - `year` (integer, valikuline)
      - `category` (text, kohustuslik)
      - `image` (text, valikuline)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)

  2. Turvalisus
    - Luba RLS portfolio_items tabelil
    - Lisa poliitikad autenditud kasutajatele haldamiseks
    - Lisa poliitikad avalikuks lugemiseks
*/
...
-- Loo updated_at trigger
CREATE TRIGGER update_portfolio_items_updated_at
  BEFORE UPDATE ON portfolio_items
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- Lisa mõned näidisandmed
