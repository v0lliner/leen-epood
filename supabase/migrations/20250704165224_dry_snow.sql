/*
  ...
  1. Storage Configuration
    ...
    - Creates appropriate storage policies
...
-- Clean up and recreate storage policies using the correct approach
-- First, drop existing policies for the bucket
BEGIN;
  -- Get policy IDs for the product-images bucket
  DO $$
  DECLARE
    policy_id text;
  BEGIN
    FOR policy_id IN 
      SELECT id FROM storage.policies 
      WHERE bucket_id = 'product-images'
    LOOP
      -- Use dynamic SQL to drop each policy
      EXECUTE format('DROP POLICY IF EXISTS %I ON storage.objects', policy_id);
    END LOOP;
  END
  $$;
COMMIT;
